...
-   面板视图窗口(panels viewports)包括图形的主要部分：条状标签、坐标轴和分面面板。视图窗口是根据它们在图形中的作用和位置来命名的。前缀描述窗口的内容，后跟整数指代x和y的位置（从左下角算起），中间以下划线"\_"分隔开（如下所列）。图C.1表示的是一个2×2图形的命名规则。
