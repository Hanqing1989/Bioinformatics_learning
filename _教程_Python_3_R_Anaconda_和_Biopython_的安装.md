---
Date created: 2022-11-11 13:40
Date modified: 2022-11-11 15:19
---

# 【教程】Python 3、R、Anaconda 和 Biopython 的安装
...
- （**在 Anaconda 创建新环境之后再进行此步**）安装完成后配置环境：
  - 在 Terminal 中输入命令：`conda info --envs`，复制 python_and_R 的路径地址到空白记事本。
  - 在 Terminal 中输入命令：`which python3`，查看该路径是否与 python_and_R 的路径相同，如不一致，继续下一步。
